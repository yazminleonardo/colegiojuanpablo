...
<body>
    <div class="container">
        <div class="container form-background">
            <h1>Registro</h1>
            <form action="" id="formLogin" method="post" name="fp" class="row g-3 needs-validation" novalidate>
                <div class="col-md-8">
                ...
                <div class="col-md-6">
                    ...
                    <select class="" name="cantidad_alumnos" id="cantidad_alumnos" onchange="cantidad_tutorados(this)" required>
                        <option selected disabled value="">Selecciona una opción</option>
...
<script type="text/javascript">
   ...
   var grado = $.trim($("#grado_estudios").val());
   var domicilio =$.trim($("#domicilio").val());
   var ocupacion =$.trim($("#ocupacion").val());
   var tipof =$.trim($("#tipof").val());
   var matricula1 =$.trim($("#matricula_alumno1").val());
   var matricula2 =$.trim($("#matricula_alumno2").val());
   var password =$.trim($("#password").val());  
   var password1 =$.trim($("#password1").val());
   var cantidad_alumnos=$.trim($("#cantidad_alumnos").val());
    ...
    if(password.length == "" || password1 == ""){
        ...
        $.ajax({
            url:"bd/registrar_tutor.php",
            type:"POST",
            datatype: "json",
            data: {nombre:nombre, apellidop:apellidop, apellidom:apellidom, curp:curp, fecha:fecha, telefono:telefono, correo:correo, grado:grado, domicilio:domicilio, ocupacion:ocupacion, tipof:tipof, matricula1:matricula1, password1:password1}, 
    ...
    if(password.length == "" || password1 == ""){
        ...
        if (password == password1) {
        $.ajax({
            url:"bd/registrar_tutor2.php",
            type:"POST",
            datatype: "json",
            data: {nombre:nombre, apellidop:apellidop, apellidom:apellidom, curp:curp, fecha:fecha, telefono:telefono, correo:correo, grado:grado, domicilio:domicilio, ocupacion:ocupacion, tipof:tipof, matricula1:matricula1, matricula2:matricula2, password1:password1}, 
            success:function(data){     
                if(data){        
                    Swal.fire({
                        position: 'center',
