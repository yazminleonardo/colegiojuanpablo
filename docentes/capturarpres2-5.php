...
<div class="container">
    ...
    $alumnos2 = "SELECT * FROM alumnos WHERE grado IN ($grado2) AND nivel = '$nivel_filtrado' AND grupo = '$grupo_filtrado'";
    ...
    $stmt->bind_param("iiiss", $grado2, $id_campo, $numero, $nivel_filtrado, $grupo_filtrado);
    $stmt->execute();
    $teku = $stmt->get_result();

    // Consulta para obtener todos los alumnos del grado, nivel y grupo específico
    $alumnos = mysqli_query($conexion, "SELECT Id_alumno FROM alumnos WHERE grado = $grado2 AND nivel = '$nivel_filtrado' AND grupo = '$grupo_filtrado'");
    ...
    if (isset($_POST['calificar'])) {
        ...
        <script>
            ...
            window.location.href = "capturapres2-5.php";
        </script>
        <?php
    }
    ?>
