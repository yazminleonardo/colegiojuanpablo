...
<?php if (isset($_POST['solicitud'])) { ?>
    <form method="post" action="reportegeneral6.pdf.php" class="" target="_blank">
        <input type="hidden" name="periodo" value="<?php if (isset($_POST['periodo'])) { print($_POST['periodo']); } ?>"></input>
        <input type="hidden" name="grado6" value="<?php if (isset($_SESSION['grado6'])) { print($_SESSION['grado6']); } ?>"></input>
...
if (isset($_POST['consultarAlumno']) && !empty($_POST['matricula'])) {
    ...
    // Consulta para obtener las calificaciones del alumno por matrícula, filtrando por Grupo A
    $vista = $conexion->prepare("SELECT alumnos.Matricula, alumnos.Nombre, alumnos.Apellidop, alumnos.Apellidom, 
                                 ...
                                 WHERE alumnos.Matricula = ? AND calificaciones.id_periodo = ? AND alumnos.grupo = 'A'
    ...
    if (empty($calificaciones)) {
        ...
        ?>
        <br>
        <form action="reportes6.pdf.php" method="post" target="_blank">
...
if (isset($_POST['solicitud'])) {
    ...
    $vista = $conexion->prepare("SELECT alumnos.Matricula, alumnos.Nombre, alumnos.Apellidop, alumnos.Apellidom, 
                                 ...
                                 INNER JOIN alumnos ON calificaciones.Id_alumno = alumnos.Id_alumno
                                 INNER JOIN campos ON calificaciones.id_asignatura = campos.id_asignatura
                                 WHERE grado = 6 AND id_periodo = ? AND alumnos.grupo = 'A'
                                 ORDER BY Apellidop");
