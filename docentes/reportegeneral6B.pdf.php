...
class ReporteCalificaciones extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(0, 6, codificarUTF8('REPORTE DE CALIFICACIONES - GRADO 6 (GRUPO B)'), 0, 1, 'C');
    ...
    function Contenido()
    {
        ...
        // Consulta SQL para obtener las calificaciones del grado 6 y grupo B
        ...
        if ($resultado->num_rows === 0) {
            $this->Cell(0, 10, "No se encontraron calificaciones para el grado 6 (Grupo B) en el trimestre seleccionado.", 0, 1, 'C');
            return;
        }
        ...
        $this->SetFont('Arial', 'B', 6); // Reducir tamaño de la fuente
        $this->SetX(10);
        $this->Cell(15, 6, codificarUTF8("Matrícula"), 1, 0, 'C'); // Reducir ancho de la celda
        $this->Cell(40, 6, codificarUTF8("Nombre"), 1, 0, 'C'); // Reducir ancho de la celda

        foreach ($asignaturas as $asignatura) {
            $this->Cell(17, 6, codificarUTF8($asignatura), 1, 0, 'C'); // Reducir ancho de la celda
        }
        ...
        $this->SetFont('Arial', '', 6); // Reducir tamaño de la fuente
        foreach ($alumnos as $matricula => $datos) {
            $this->SetX(10);
            $this->Cell(15, 6, $matricula, 1); // Reducir ancho de la celda
            $this->Cell(40, 6, codificarUTF8($datos['Nombre']), 1); // Reducir ancho de la celda
            ...
            foreach ($asignaturas as $asignatura) {
                ...
                $this->Cell(17, 6, $calificacion, 1, 0, 'C'); // Reducir ancho de la celda
...
$reporte->Output('reporte_calificaciones_grado6_grupoB.pdf', 'I');
?>