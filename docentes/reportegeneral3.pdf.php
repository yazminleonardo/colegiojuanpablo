...
class ReporteCalificaciones extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(0, 6, codificarUTF8('REPORTE DE CALIFICACIONES - GRADO 3 (GRUPO A)'), 0, 1, 'C');
    ...
    function Contenido()
    {
        ...
        // Consulta SQL para obtener las calificaciones del grado 3 y grupo A
        $stmt = $conexion->prepare("SELECT alumnos.Matricula, alumnos.Nombre, alumnos.Apellidop, alumnos.Apellidom,
                                    ...
                                    WHERE alumnos.grado = 3 AND calificaciones.id_periodo = ? AND alumnos.grupo = 'A'
        ...
        if ($resultado->num_rows === 0) {
            $this->Cell(0, 10, "No se encontraron calificaciones para el grado 3 (Grupo A) en el trimestre seleccionado.", 0, 1, 'C');
            return;
        }
...
$reporte->Output('reporte_calificaciones_grado3_grupoA.pdf', 'I');
?>