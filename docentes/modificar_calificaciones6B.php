...
<form method="post" class="d-flex row g-3">
    ...
    <div class="col-md-3">
        ...
        <select name="asignatura" id="" class="form-select">
            ...
            $filtro_asignaturas = "id_asignatura IN (12, 13, 14, 15, 38, 16, 17, 41, 40, 19, 20, 21, 22)"; // Cambia esto según tus necesidades
...
if (isset($_POST['solicitud'])) {
    ...
    $grado5 = $_SESSION['grado5'];
    $grado6 = $_SESSION['grado6'];
    $asignatura = $_POST['asignatura'];

    // Consulta SQL modificada para usar la tabla `calificaciones`
    $vista = $conexion->prepare("SELECT alumnos.Id_alumno, alumnos.Matricula, calificaciones.id_calificacion, alumnos.Nombre, alumnos.Apellidop, Apellidom, campos.campo_formativo, calificaciones.calificacion, campos.id_asignatura
        ...
        WHERE calificaciones.id_asignatura = ? AND Matricula = ? AND grado = ? AND id_periodo = ? 
        ORDER BY Apellidop");
    
    $vista->bind_param("isii", $asignatura, $matricula, $grado6, $periodo);
    $vista->execute();
    $resultados = $vista->get_result();
?>
<br>
<label class="form-label fw-bold">Trimestre: <?php echo "$periodo" ?>° Trimestre</label><br>
<label class="form-label fw-bold">Grado: <?php echo "$grado6" ?>°</label><br>
<label class="form-label fw-bold">Grupo: B</label>
<br>
<form action="bd/solicitud6B.php" method="POST">
    <table class="content-table table">
        ...
        <tbody>
            ...
            foreach ($resultados as $alumno) {
                ...
                echo '<input type="hidden" name="id_calificacion" value="' . $alumno['id_calificacion'] . '">';
