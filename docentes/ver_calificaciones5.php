...
<?php if (isset($_POST['solicitud'])) { ?>
    <form method="post" action="reportegeneral5.pdf.php" class="" target="_blank">
        <input type="hidden" name="periodo" value="<?php if (isset($_POST['periodo'])) { print($_POST['periodo']); } ?>"></input>
        <input type="hidden" name="grado5" value="<?php if (isset($_SESSION['grado5'])) { print($_SESSION['grado5']); } ?>"></input>
...
if (isset($_POST['consultarAlumno']) && !empty($_POST['matricula'])) {
    ...
    $vista = $conexion->prepare("SELECT alumnos.Matricula, alumnos.Nombre, alumnos.Apellidop, alumnos.Apellidom, 
                                 ...
                                 WHERE alumnos.Matricula = ? AND calificaciones.id_periodo = ? AND alumnos.grado = 5
    ...
    if (empty($calificaciones)) {
        ...
        ?>
        <br>
        <form action="reportes5.pdf.php" method="post" target="_blank">
                ...
                <tbody>
                    <tr>
                        ...
                        foreach ($asignaturas as $asignatura) {
                            ...
                            echo '<tdclass="vert-text">' . htmlspecialchars($calificacion) . '</td>';
                        }
...
if (isset($_POST['solicitud'])) {
    $periodo = $_POST['periodo'];
    //$grado5 = $_SESSION['grado5']; // Cambié esta línea para que use grado 5
    ...
    $vista = $conexion->prepare("SELECT alumnos.Matricula, alumnos.Nombre, alumnos.Apellidop, alumnos.Apellidom, 
                                 ...
                                 INNER JOIN alumnos ON calificaciones.Id_alumno = alumnos.Id_alumno
                                 INNER JOIN campos ON calificaciones.id_asignatura = campos.id_asignatura
                                 WHERE alumnos.grado = 5 AND calificaciones.id_periodo = ? 
                                 ORDER BY alumnos.Apellidop");
                ...
                foreach ($resultado as $row) { 
                    if ($row['Matricula'] != $alumno_actual) {
                        ...
                        foreach ($asignaturas as $asignatura) {
                            ...
                            echo '< class="vert-text"td>' . $calificacion . '</td>';
                        }
