...
<div class="col-md-3">
    ...
    <option value="2">Saberes y pensamiento cientifico</option>
    <option value="3">Ética naturaleza y sociedad</option>
    <option value="4">De lo humano a lo comunitario</option>
    </select>
</div>
    <div class="col-md-4">
...
if (isset($_POST['solicitud'])) {
    ...
    //echo 'El periodo es:' . $periodo; echo 'El grado1 es:' . $grado1; echo 'El grado2 es:' . $grado2;
    $vista = $conexion->prepare("SELECT alumnos.Id_alumno, alumnos.Matricula, calificaciones.id_calificacion,alumnos.Nombre, alumnos.Apellidop, Apellidom, campos.campo_formativo, calificaciones.calificacion,campos.id_asignatura
        FROM calificaciones
        INNER JOIN alumnos ON calificaciones.Id_alumno = alumnos.Id_alumno
        INNER JOIN campos ON calificaciones.id_asignatura = campos.id_asignatura WHERE  calificaciones.id_asignatura=$asignatura And Matricula='$matricula' AND grado=$grado1 AND id_periodo=$periodo order BY Apellidop");
...
<br>
<form action="bd/solicitudpres1.php" method="POST">
    <table class="content-table table">
        ...
        <tbody>
            ...
            foreach ($resultados as $alumno) {
                ...
                echo'<input type="hidden" name="id_calificacion" value="'.$row['id_calificacion'].'">';
                echo'<input type="hidden" name="asignatura" value="'.$row['id_asignatura'].'">';
