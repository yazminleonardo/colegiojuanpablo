...
class ReporteCalificaciones extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(0, 6, codificarUTF8('REPORTE DE CALIFICACIONES - GRADO 1 (GRUPO B)'), 0, 1, 'C');
    ...
    function Contenido()
    {
        ...
        // Consulta SQL para obtener las calificaciones del grado 1 y grupo B
        $stmt = $conexion->prepare("SELECT alumnos.Matricula, alumnos.Nombre, alumnos.Apellidop, alumnos.Apellidom,
                                    ...
                                    WHERE alumnos.grado = 1 AND calificaciones.id_periodo = ? AND alumnos.grupo = 'B'
        ...
        if ($resultado->num_rows === 0) {
            $this->Cell(0, 10, "No se encontraron calificaciones para el grado 1 (Grupo B) en el trimestre seleccionado.", 0, 1, 'C');
            return;
        }
        ...
        foreach ($asignaturas as $asignatura) {
            $this->Cell(19, 6, codificarUTF8($asignatura), 1, 0, 'C'); // Reducir ancho de la celda
        }
        ...
        foreach ($alumnos as $matricula => $datos) {
            ...
            foreach ($asignaturas as $asignatura) {
                ...
                $this->Cell(19, 6, $calificacion, 1, 0, 'C'); // Reducir ancho de la celda
...
$reporte->Output('reporte_calificaciones_grado1_grupoB.pdf', 'I');
?>