...
if (isset($_POST['periodo'])) {
    ...
    $sheet->setCellValue("A1", "COLEGIO JUAN PABLO II    3°    PRIMARIA");
        ...
        if ($row['Matricula'] != $alumno_actual) {
            ...
            for ($i = 0; $i < $totalMaterias; $i++) {
                ...
                if ($i == 2) {
                    ...
                    $promedio3 = ($totalMaterias >= 8) ? ( ($valores[6] + $valores[7]) / 2 ) : null;
                    $sheet->setCellValue($col . $fila, number_format($promedio3, 2));
                    $col++;
                }elseif ($i == 11) {
                    $promedio3 = ($totalMaterias >= 12) ? ( ($valores[8] + $valores[9] + $valores[10] + $valores[11]) / 4 ) : null;
                    $sheet->setCellValue($col . $fila, number_format($promedio3, 2));
                    $col++;
                }
            }
    ...
    $sheet->getStyle('C3:R3')->getAlignment()->setTextRotation(90);
    $sheet->getStyle('C3:R3')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
    $sheet->getStyle('C3:R3')->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
    ...
    $rangoTabla = "A1:R" . ($fila - 1); // Rango de la tabla
    $styleArray = [
        'borders' => [
            'allBorders' => [
