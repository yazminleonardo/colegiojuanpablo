...
<label class="form-label h5 fw-bold mt-3">PREESCOLAR 1°</label></BR>
...
<?php if (isset($_POST['solicitud'])) { ?>
    <form method="post" action="reportes1.excel.php" class="" target="_blank">
...
if (isset($_POST['consultarAlumno']) && !empty($_POST['matricula'])) {
    ...
    $vista = $conexion->prepare("SELECT alumnos.Matricula, alumnos.Nombre, alumnos.Apellidop, alumnos.Apellidom, 
                                 ...
                                 WHERE grado = 1 AND prescolar.id_periodo = ? AND alumnos.grupo = 'A'
