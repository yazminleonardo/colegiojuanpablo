...
if (isset($_POST['periodo'])) {
        ...
        if ($row['Matricula'] != $alumno_actual) {
            ...
            for ($i = 0; $i < $totalMaterias; $i++) {
                ...
                if ($i == 2) {
                    ...
                    $promedio3 = ($totalMaterias >= 8) ? ( ($valores[5] + $valores[6] + $valores[7]) / 3 ) : null;
                    $sheet->setCellValue($col . $fila, number_format($promedio3, 2));
                    $col++;
                }elseif ($i == 9) {
                    $promedio3 = ($totalMaterias >= 10) ? ( ($valores[8] + $valores[9]) / 2 ) : null;
                    $sheet->setCellValue($col . $fila, number_format($promedio3, 2));
                    $col++;
                }
            }
    ...
    $sheet->getStyle('C3:P3')->getAlignment()->setTextRotation(90);
    $sheet->getStyle('C3:P3')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
    $sheet->getStyle('C3:P3')->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
    $sheet->getStyle('C:P')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
    $sheet->getStyle('C:P')->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
    ...
    $rangoTabla = "A1:P" . ($fila - 1); // Rango de la tabla
    $styleArray = [
        'borders' => [
            'allBorders' => [
