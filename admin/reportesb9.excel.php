...
if (isset($_POST['periodo'])) {
    ...
    $stmt = $conexion->prepare(" 
        ...
        WHERE alumnos.grado = 6 AND calificaciones.id_periodo = ? AND alumnos.grupo = 'B'
    ...
    $sheet->setCellValue("A1", "COLEGIO JUAN PABLO II    6°B    PRIMARIA");
    ...
    $sheet->setCellValue("K2", "ÉTICA\nNATURALEZA Y\n SOCIEDAD");
    $sheet->setCellValue("O2", "DE LO HUMANO Y LO\nCOMUNITARIO");

    // Configurar el ajuste de texto para que el salto de línea se vea correctamente
    foreach (["C2", "G2", "K2", "O2"] as $cell) {
    ...
    /*if (count($asignaturas) >= 4) {
        $sheet->setCellValue($col . '3', 'Promedio');
        $sheet->getStyle($cell)->getFont()->setBold(true); // Aplica negrita
        $col++;
    }*/
        ...
        if ($row['Matricula'] != $alumno_actual) {
            ...
            foreach ($datos as $cal_row) {
                if ($cal_row['Matricula'] == $alumno_actual) {
                    $calificaciones[$cal_row['campo_formativo']] = floatval($cal_row['calificacion']);
                }
            }
            ...
            for ($i = 0; $i < $totalMaterias; $i++) {
                ...
                if ($i == 2) {
                    ...
                    $promedio2 = ($totalMaterias >= 6) ? ( ($valores[3] + $valores[4] + $valores[5]) / 3 ) : null;
                    $sheet->setCellValue($col . $fila, number_format($promedio2, 2));
                    $col++;
                } elseif ($i == 8) {
                    $promedio3 = ($totalMaterias >= 9) ? ( ($valores[6] + $valores[7] + $valores[8]) / 3 ) : null;
                    $sheet->setCellValue($col . $fila, number_format($promedio3, 2));
                    $col++;
                } elseif ($i == 12) {
                    $promedio3 = ($totalMaterias >= 13) ? ( ($valores[9] + $valores[10] + $valores[11] + $valores[12]) / 4 ) : null;
                    $sheet->setCellValue($col . $fila, number_format($promedio3, 2));
                    $col++;
                }
            }
    ...
    $sheet->getStyle('C3:S3')->getAlignment()->setTextRotation(90);
    $sheet->getStyle('C3:S3')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
    $sheet->getStyle('C3:S3')->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
    $sheet->getStyle('C:S')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
    $sheet->getStyle('C:S')->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
    ...
    $rangoTabla = "A1:S" . ($fila - 1); // Rango de la tabla
    $styleArray = [
        'borders' => [
            'allBorders' => [
