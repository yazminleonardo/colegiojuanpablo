...
$vista = $conexion->prepare("
    SELECT 
        alumnos.Id_alumno, prescolar.id_calificaciones, alumnos.Matricula, alumnos.Nombre, 
        alumnos.Apellidop, alumnos.Apellidom, campos.campo_formativo, 
        prescolar.calificacion, modificarpres.calificacion, 
        modificarpres.motivo, modificarpres.id_periodo
    FROM modificarpres
    INNER JOIN prescolar ON modificarpres.id_calificacion = prescolar.id_calificaciones
    INNER JOIN alumnos ON modificarpres.id_alumno = alumnos.Id_alumno
    INNER JOIN campos ON modificarpres.id_asignatura = campos.id_asignatura 
    WHERE estado='Espera';
");
...
$revision = mysqli_query($conexion, "SELECT id FROM modificarpres WHERE estado='Espera'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['aprobar']) || isset($_POST['rechazar'])) {
        ...
        if (isset($_POST['aprobar'])) {
            $aceptado = mysqli_query($conexion, "UPDATE prescolar SET calificaciones=$calificacion WHERE id_calificacion=$id");
            $cambio = mysqli_query($conexion, "UPDATE modificarpres SET estado='Aprobado' WHERE id_calificacion=$id");
        } elseif (isset($_POST['rechazar'])) {
            $cambio = mysqli_query($conexion, "UPDATE modificarpres SET estado='Rechazado' WHERE id_calificacion=$id");
        }
    ...
    <table class="content-table table table-bordered">
        ...
        <?php foreach ($resultados as $row): ?>
            <tr>
                ...
                <td><?= $row['motivo']; ?></td>
                <td>
                    <input type="hidden" name="xyz" value="<?= $row['id_calificacion']; ?>">
